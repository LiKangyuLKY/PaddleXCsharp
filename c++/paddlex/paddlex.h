...
class Model {
 public:
  ...
  int paddlex_init(const std::string& model_dir,
            bool use_gpu = false,
            bool use_trt = false,
            bool use_mkl = true,
            int mkl_thread_num = 4,
            ...
            std::string key = "",
            bool use_ir_optim = true);
  bool load_score_thresholds_config(const std::string& yaml_input);
  bool paddlex_predict(const cv::Mat& im, ClsResult* result);
  bool paddlex_predict(const cv::Mat& im, DetResult* result);
  bool paddlex_predict(const cv::Mat& im, SegResult* result);
//  bool paddlex_predict(unsigned char *img, int height, int width, int channels, ClsResult* result);
//  bool paddlex_predict(unsigned char *img, int height, int width, int channels, DetResult* result);
//  bool paddlex_predict(unsigned char *img, int height, int width, int channels, SegResult* result);
