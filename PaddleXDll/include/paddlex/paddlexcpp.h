...
#pragma pack ()
	...
	入参:
		...
		width:图片宽度(像素)
		channels:图片通道数
		result:返回的推理结果，内部结构为本文件中定义的一字节对齐的"t_cls_result"结构体
	返回值:
		推理是否成功
	*/
	extern "C" __declspec(dllexport) bool __cdecl PaddlexClsPredict(void* model, unsigned char *img, int height, int width, int channels, char **result);
	...
	入参:
		...
		width:图片宽度(像素)
		channels:图片通道数
		result:返回的推理结果，内部结构为:"flost类型的box_num"+多个"本文件中定义的一字节对齐的t_det_result结构体"
		visualize:是否进行结果可视化。默认进行可视化，即在原图img上画出预测的box框。
	返回值:
		推理是否成功
	*/
	extern "C" __declspec(dllexport) bool __cdecl PaddlexDetPredict(void* model, unsigned char *img, int height, int width, int channels, char **result, bool visualize = true);
